...
{
    public function incrementAssetRentCountCTRL(): void
    {
        ...
        $updated = $model->incrementRentCount((int) $assetId);

        if ($updated) {
            self::jsonResponse(['error' => false, 'message' => 'Rent count updated successfully.']);
        } else {
...
(new AssetStatsCTRL())->incrementAssetRentCountCTRL();
