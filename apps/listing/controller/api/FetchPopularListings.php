...
{
    public function selectPopularAssetsCTRL(): void
    {
        ...
        $results = $model->select_popular_assets();

        if (!empty($results)) {
            self::jsonResponse(['error' => false, 'data' => $results]);
        } else {
            self::jsonResponse(['error' => true, 'message' => 'No popular assets found.'], 404);
        }
    }
...
(new FetchAllCTRL())->selectPopularAssetsCTRL();
