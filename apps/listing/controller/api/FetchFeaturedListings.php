...
{
    public function selectFeaturedAssetsCTRL(): void
    {
        ...
        $results = $model->select_featured_assets_random_ten();

        if (!empty($results)) {
            self::jsonResponse(['error' => false, 'data' => $results]);
        } else {
            self::jsonResponse(['error' => true, 'message' => 'No featured assets found.'], 404);
        }
    }
...
(new FetchAllCTRL())->selectFeaturedAssetsCTRL();
